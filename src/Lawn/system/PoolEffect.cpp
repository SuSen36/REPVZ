...
void PoolEffect::PoolEffectInitialize()
{
    ...
    mCausticImage->mBits = new uint32_t[CAUSTIC_IMAGE_WIDTH * CAUSTIC_IMAGE_HEIGHT];
        ...
        for (int y = 0; y < 5; y++)
        {
            for (int aLayer = 0; aLayer < 3; aLayer++)
            {
                ...
                for (int aVertIndex = 0; aVertIndex < 6; aVertIndex++, pVert++)
                {
                    ...
                    if (aLayer == 2)
                    {
                        pVert->x = (704.0f / 15.0f) * aIndexX + 45.0f + 220.0f;
                    ...
                    else
                    {
                        ...
                        pVert->x = aIndexX * aGridSquareX + 35.0f + 220.0f;
