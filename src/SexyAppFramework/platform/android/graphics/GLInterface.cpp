...
static void CopyImageToTexture8888(MemoryImage *theImage, int offx, int offy, int theWidth, int theHeight, int theDestPitch, int theDestHeight, bool rightPad, bool bottomPad, bool create)
{
	...
	if (create)
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, theDestPitch, theDestHeight, 0, GL_BGRA_EXT, GL_UNSIGNED_BYTE, aDest);
	else
		glTexSubImage2D(GL_TEXTURE_2D, 0, offx, offy, theDestPitch, theDestHeight, GL_BGRA_EXT, GL_UNSIGNED_BYTE, aDest);
...
int GLInterface::Init(bool IsWindowed)
{
    ...
    // 创建窗口
    SDL_Window* window = SDL_CreateWindow("OpenGL ES",
                                          SDL_WINDOWPOS_UNDEFINED,
                                          SDL_WINDOWPOS_UNDEFINED,
                                          mWidth, mHeight,
                                          SDL_WINDOW_OPENGL | (IsWindowed ? 0 : SDL_WINDOW_FULLSCREEN));
    ...
    gMinTextureWidth = 8;
    gMinTextureHeight = 8;
    gMaxTextureWidth = aMaxSize; // 使用查询到的最大纹理大小
    ...
    glLoadIdentity();
    SetOrtho(0, mWidth - 1, mHeight - 1, 0, -10, 10);
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();

    // 设置 OpenGL 的状态
    glEnable(GL_BLEND); // 启用混合
    glDisable(GL_DITHER); // 禁用抖动
...
bool GLInterface::RecoverBits(MemoryImage* theImage)
{
	...
	for (int aPieceRow = 0; aPieceRow < aData->mTexVecHeight; aPieceRow++)
	{
		for (int aPieceCol = 0; aPieceCol < aData->mTexVecWidth; aPieceCol++)
		{
			...
			glBindTexture(GL_TEXTURE_2D, aPiece->mTexture);
            //TODO
			//glGetTexImage(GL_TEXTURE_2D, 0, GL_BGRA_EXT, GL_UNSIGNED_BYTE, theImage->GetBits());
